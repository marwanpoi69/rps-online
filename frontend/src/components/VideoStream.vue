<template>
  <div class="bg-white bg-opacity-10 backdrop-blur-md rounded-xl p-4">
    <div class="mb-4">
      <!-- Camera selection -->
      <div v-if="isLocal && cameras.length > 0" class="mb-2">
        <select 
          v-model="selectedCamera"
          @change="setupLocalVideo"
...
import { initializeCamera, captureFrame, getVideoDevices } from '../utils/camera.js'
...
onMounted(async () => {
  if (props.isLocal) {
    ...
    cameras.value = await getVideoDevices()
...
const setupLocalVideo = async () => {
  try {
    ...
    const mediaStream = await initializeCamera(selectedCamera.value)
